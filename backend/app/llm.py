...
SYSTEM_GENERATE_OUTLINE = """你是 Word 文档大纲设计师。根据用户提供的主题，生成结构化章节大纲。

要求：
- 章节数量由主题复杂度自行决定（短函件 3-5 节、工作报告 5-8 节、长文 10-20 节）
...
def generate_outline(
    ...
    if not isinstance(outline, list):
        outline = []
    return {
        "outline": [str(x).strip() for x in outline if str(x).strip()][:25],
    }, info
...
3. suggested_options（用户可改的 4 个默认设置）：
   * language: zh / en（首选 zh，除非内容明显是英文）
   * scenario: report / contract / letter / memo / academic / general
   * audience: general / executive / team / client / expert / student
   * tone: formal / professional / technical / concise / casual

4. outline：章节大纲，数量由主题复杂度自行决定。每条一行，标题具体、有递进。

仅输出 JSON，结构固定如下，不要 markdown 围栏、不要解释、不要寒暄：
{
  "core_topic": "...",
  "key_points": ["...", "..."],
  "suggested_options": {"language": "...", "scenario": "...", "audience": "...", "tone": "..."},
  "outline": ["第一章 ...", "第二章 ..."]
}"""
