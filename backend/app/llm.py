...
SYSTEM_OPTIMIZE_PROMPT = """你是 Word 文档创作助手。根据用户提供的主题，输出一段扩展后的、适合交给 Word 生成 agent 的中文描述。

要求：
- 明确核心信息（这篇文档想传达什么）
- 列出 3-5 个关键观点（要传达给读者的要点）
- 给出建议的叙事结构（一两句话：先讲什么、后讲什么、如何收尾）
- 顺便根据主题推荐 5 个默认设置（用户可手动改）：
  * language: zh / en（首选 zh，除非主题明显是英文内容）
  * scenario: report / contract / letter / memo / academic / general
  * audience: general / executive / team / client / expert / student
  * tone: formal / professional / technical / concise / casual
  * page_count: 2-30 的整数，表示目标章节数（短函件 3-5、工作报告 5-8、综述 10-20）
...
SYSTEM_GENERATE_OUTLINE = """你是 Word 文档大纲设计师。根据用户提供的主题与目标章节数，生成结构化章节大纲。

要求：
- 章节条数尽量接近 page_count（可含封面/附录）
- 每条一行，可用「第一章」「一、」或破折号开头，便于逐行展示
- 章节标题要具体、信息密度高，避免空泛的「引言/概述/总结」
- 章节之间要有递进关系
- 语言根据传入的 language
- 仅输出 JSON：{"outline": ["第一章 ...", "第二章 ...", ...]}
不要 markdown 围栏。"""
...
_VALID_SCENARIOS = {
    "general", "report", "contract", "letter", "memo", "academic",
}
_VALID_AUDIENCES = {
    "general", "executive", "team", "client", "expert", "student",
}
_VALID_TONES = {"formal", "casual", "technical", "professional", "concise"}
...
def optimize_prompt(
    ...
    suggested_options = {
        ...
        "page_count": _pick_int(suggested_raw.get("page_count"), 2, 30, 5),
    }
...
SYSTEM_AUTOFILL_PROMPT = """你是 Word 文档创作总策划。根据用户提供的「主题」或「文档内容」，一次性产出整套创作方案，供前端自动填充各字段。

你必须在一次输出里给出以下全部内容：

1. core_topic：一句话核心主题（仅当输入是文档时需要从文档提炼；输入是主题时原样回传、可微调措辞）。≤200 字。

2. key_points：3-6 条要重点传达的要点，每条一行。

3. suggested_options（用户可改的 5 个默认设置）：
   * language: zh / en（首选 zh，除非内容明显是英文）
   * scenario: report / contract / letter / memo / academic / general
   * audience: general / executive / team / client / expert / student
   * tone: formal / professional / technical / concise / casual
   * page_count: 2-30 整数（短函 3-5、报告 5-8、长文 10-20）

4. outline：章节大纲，条数尽量接近 page_count。每条一行，标题具体、有递进。
...
SYSTEM_TEMPLATE_FILL_PROMPT = """你是 Word 模板变量填充助手。用户提供了模板占位符列表和一段素材（文字或文档摘要）。
请从素材中提取信息，填入对应变量。

规则：
1. 只输出 slots 列表中存在的 key，不要发明新 key。
2. 信息不足时：该 key 省略或给空字符串 ""，不要编造。
3. 值为可直接写入 Word 的正文，不要包含 {{}} 占位符。
4. 中文模板默认使用中文；日期、金额等按素材原样或合理格式化。
5. 长文本字段（如 content、body、contract_body）可写完整段落；短字段（如 title、date）保持简洁。

仅输出 JSON，不要 markdown 围栏、不要解释：
{
  "template_data": {
    "key1": "值1",
    "key2": "值2"
...
def template_slot_fill(
    *,
    ...
    """Map source material to template slot values.

    slots: [{key, label, hint?}, ...]
    Returns {template_data, filled_keys, missing_keys}.
    """
    if not slots:
        return {"template_data": {}, "filled_keys": [], "missing_keys": []}, {}
    ...
    for s in slots:
        ...
        slot_lines.append(f"- {key}: {label}{hint_str}")
    ...
    if is_document:
        user = (
            ...
            f"需要填充的变量：\n{slots_block}\n\n"
            f"用户上传的文档内容（markdown，可能截断）：\n\n{seed}\n\n"
            f"请从文档内容提取并填入 template_data。"
        )
    else:
        user = (
            f"模板名称：{template_name}\n\n"
            f"需要填充的变量：\n{slots_block}\n\n"
            f"用户提供的相关信息：\n{seed}\n\n"
            f"请从上述信息提取并填入 template_data。"
        )

    parsed, info = chat_json(SYSTEM_TEMPLATE_FILL_PROMPT, user, max_tokens=4000)

    raw_data = parsed.get("template_data") or {}
    ...
    for key in valid_keys:
        ...
        text = str(val).strip()
        if text:
            template_data[key] = text[:8000]

    filled_keys = list(template_data.keys())
    missing_keys = [k for k in valid_keys if k not in template_data]
