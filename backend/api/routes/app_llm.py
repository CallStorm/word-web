...
async def post_template_fill(
    user: CurrentUser,
    mode: Annotated[str, Form()] = "topic",
    source_text: Annotated[str, Form()] = "",
    template_name: Annotated[str, Form()] = "",
    slots_json: Annotated[str, Form()] = "[]",
    files: Annotated[list[UploadFile], File()] = [],
) -> dict:
    """Map user source material to template slot values via LLM."""
    try:
        raw_slots = json.loads(slots_json) if slots_json.strip() else []
    except json.JSONDecodeError as e:
        raise HTTPException(
            status_code=422,
            detail={"code": "invalid_slots", "message": "slots_json must be valid JSON array"},
        ) from e
    if not isinstance(raw_slots, list):
        raise HTTPException(
            status_code=422,
            detail={"code": "invalid_slots", "message": "slots_json must be a JSON array"},
        )

    slots = [s for s in raw_slots if isinstance(s, dict) and s.get("key")]
    ...
    if mode == "document":
        ...
        try:
            data, info = await asyncio.to_thread(
                llm.template_slot_fill,
                template_name=template_name.strip() or "模板",
                slots=slots,
                seed=seed,
                is_document=True,
            )
    ...
    seed = source_text.strip()
    if not seed and not files:
        raise HTTPException(
            status_code=422,
            detail={"code": "empty_source", "message": "请先输入相关信息或上传参考文件"},
        )

    if not seed and files:
        read_files = []
        for f in files:
            content = await f.read()
            read_files.append((f.filename or "upload", content))
